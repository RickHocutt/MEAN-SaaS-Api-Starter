System.register(['angular2/core', '../services/logger.service', '../helpers/is-admin', '../services/role.service', '../models/role', 'angular2/router'], function(exports_1, context_1) {
    ...
    return {
        ...
        execute: function() {
            EditRoleComponent = (function () {
                function EditRoleComponent(_service, _logger, _params) {
                    var _this = this;
                    ...
                    this._service.getRole(this.roleId)
                        .subscribe(function (doc) {
                        if (!doc.error) {
                            _this._logger.log('Retrieved role ' + _this.roleId);
                            _this.model = doc.data;
                        }
                        else {
                            _this._logger.log('Unable to find user ' + _this.roleId);
                        }
                    }, function (err) { return _this._logger.log('Unable to find user ' + _this.roleId); });
                }
                EditRoleComponent = __decorate([
                    ...
                    __metadata('design:paramtypes', [role_service_1.RoleService, logger_service_1.Logger, router_1.RouteParams])
                ], EditRoleComponent);
                return EditRoleComponent;
            }());
            exports_1("EditRoleComponent", EditRoleComponent);
        }
    }
});
